...
const arrayImpar = randomNumbers.filter((randomNumber)=> randomNumber %2 === 1)
...
const numberDown501 = crazyNumbers.filter((crazyNumber)=> crazyNumber <= 501)
console.log(numberDown501)
...
const numbersPow = numbers.map((number)=> Math.pow(number , 2))
...
const tarantinoMovies2000 = tarantinoMovies.filter((tarantinoMovie)=> tarantinoMovie.release <=2000)
console.log(tarantinoMovies2000);
...
const nameSeries = tvShows.map((tvShow)=>tvShow.name)
...
const productList = cart.reduce((accumulator, {name})=>
    `${accumulator}- ${name}\n`)
    const productString = productList.toString()
